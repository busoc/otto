...
select
	...
	count(replay_id) as total
from replay_job
	where replay_status_id=(select id from replay_status where workflow=(select workflow from pending)) group by time
union all
select
	'CANCELLED' as label,
	date(timestamp) as time,
	count(replay_id) as total
from replay_job
	where replay_status_id=(select id from replay_status where workflow=(select workflow from cancelled)) group by time
union all
select
	'COMPLETED' as label,
	date(timestamp) as time,
	count(replay_id) as total
from replay_job
	where replay_status_id in (select id from replay_status where workflow in (select workflow from completed)) group by time
union all
select
	'RUNNING' as label,
	date as time,
	count(replay_status_id) as total
from (
	select
	...
	from replay_job
		where replay_status_id in (select workflow from running) group by replay_id, date) as g
group by date
